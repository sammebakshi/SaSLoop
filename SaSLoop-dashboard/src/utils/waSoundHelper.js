// Helper for playing WhatsApp new message notification sounds in Dashboard
...
function createWavDataUri(f1, f2 = 0, duration = 0.6) {
  try {
    ...
    console.error("[WA Sound] Failed to create WAV Data URI:", e);
    return '';
  }
}

export function playWhatsAppNotificationSound(soundType = 'default', customUrl = '') {
  try {
    const type = soundType || 'default';

    if (type === 'custom' && customUrl) {
      const audio = new Audio(customUrl);
      audio.play().catch(e => console.warn("[WA Sound] Failed to play custom sound:", e));
      return;
    }

    // Attempt Web Audio API first if context is active
    const ctx = getAudioContext();
    if (ctx) {
      ctx.resume().catch(() => {});
    ...
    if (dataUri) {
      ...
      audio.play().catch(e => console.warn("[WA Sound] HTML5 Audio fallback failed:", e));
    }
  } catch (e) {
    console.warn("[WA Sound] Error playing message notification sound:", e);
  }
}

export function playConfiguredWaSound() {
  if (typeof window === 'undefined') return;
  const isEnabled = localStorage.getItem('wa_sound_enabled') !== 'false';
  if (!isEnabled) return;

  const soundType = localStorage.getItem('wa_sound_type') || 'default';
  const customUrl = localStorage.getItem('wa_custom_sound') || '';
  playWhatsAppNotificationSound(soundType, customUrl);
}

export function getWaSoundSettings() {
  if (typeof window === 'undefined') return { enabled: true, soundType: 'default', customSoundName: '' };
  return {
    enabled: localStorage.getItem('wa_sound_enabled') !== 'false',
    soundType: localStorage.getItem('wa_sound_type') || 'default',
    customSoundName: localStorage.getItem('wa_custom_sound_name') || ''
  };
}
