...
import { 
  Warehouse, Search, RefreshCw, Plus, Edit3, Trash2, 
  X, Check, Building2, Box
} from "lucide-react";
...
const WarehouseManager = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/inventory/locations${q}`, {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const q = getImpersonateParam();
            const body = { name, code, description, is_active: isActive };
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const q = getImpersonateParam();
            const res = await fetch(`${API_BASE}/api/inventory/locations/${id}${q}`, {
                method: "DELETE",
                headers: getAuthHeaders()
            });
            if (res.ok) fetchData();
    ...
    return (
        <div className="space-y-4 animate-pro-in pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-[#1e2129] p-4 rounded-xl border border-slate-100 dark:border-white/5 shadow-sm">
                <div>
                    <h2 className="text-[18px] font-black text-slate-900 dark:text-white uppercase tracking-tight">Storage Warehouses & Locations</h2>
                    <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Organize raw materials across Main Kitchen, Dry Store, Cold Rooms & Pantry</p>
                </div>
            ...
            <div className="bg-white dark:bg-[#1e2129] rounded-xl border border-slate-100 dark:border-white/5 overflow-hidden shadow-sm">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-[11px] font-bold text-slate-800 dark:text-slate-200">
                        {loading ? (
                            <tr><td colSpan="5" className="p-12 text-center text-slate-400 animate-pulse">Loading locations...</td></tr>
                        ) : filtered.length === 0 ? (
                            <tr><td colSpan="5" className="p-12 text-center text-slate-400">No storage locations found.</td></tr>
                        ) : filtered.map(loc => (
                            <tr key={loc.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                <td className="p-3.5">
                                    <div className="flex items-center gap-2.5">
                                        <div className="w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-500 flex items-center justify-center">
                                            <Building2 className="w-4 h-4" />
                                        </div>
                                        <span className="font-black text-slate-900 dark:text-white uppercase tracking-tight">{loc.name}</span>
                                    </div>
                                </td>
                                <td className="p-3.5">
                                ...
                                <td className="p-3.5 text-slate-500">{loc.description || "N/A"}</td>
                                <td className="p-3.5">
                                    <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${loc.is_active !== false ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-100 text-slate-400'}`}>
                                        {loc.is_active !== false ? 'Active' : 'Disabled'}
                                    </span>
                                </td>
                                <td className="p-3.5 text-right">
                                    <div className="flex items-center justify-end gap-1.5">
                                        <button onClick={() => handleOpenModal(loc)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white">
                                            <Edit3 className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDelete(loc.id)} className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg text-rose-400 hover:text-rose-600">
                ...
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-[#1e2129] border border-slate-100 dark:border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSave} className="space-y-4 mt-4 text-[11px] font-bold text-slate-700 dark:text-slate-300">
                            <div>
                                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Location Name *</label>
                                <input 
                                    type="text" 
                                    required
                                    value={name} 
                                    onChange={(e) => setName(e.target.value)} 
                                    placeholder="e.g. Main Kitchen Store, Cold Storage"
                            ...
                            <div>
                                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Location Code</label>
                                <input 
                                    type="text" 
                                    value={code} 
                                    onChange={(e) => setCode(e.target.value)} 
                                    placeholder="e.g. WH-KIT-01"
                                    className="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 outline-none text-slate-900 dark:text-white uppercase font-bold"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder="Optional description of warehouse..."
                                    rows="2"
                            ...
                            <div className="flex items-center gap-2 pt-1">
                                ...
                                />
                                <label htmlFor="locActive" className="cursor-pointer text-[11px] font-bold text-slate-700 dark:text-slate-300">Active Location</label>
                            </div>
