...
import { 
  ...
  Landmark, CreditCard as CardIcon, Receipt, ChevronRight
} from "lucide-react";
import API_BASE from "../config";

const PhonePeTrxReport = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/brand/analytics/phonepe-transactions?${q}`, {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">PhonePe Settlement</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Settlement types, transaction identifiers & webhook temporal artifacts</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button className="px-4 py-2 bg-purple-600 text-white rounded-md font-bold text-[10px] uppercase tracking-widest hover:bg-purple-500 transition-all flex items-center gap-2 shadow-md shadow-purple-600/10">
                        <Download className="w-3.5 h-3.5" /> Export PhonePe Audit
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm relative overflow-hidden group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-1 gap-6 items-end justify-items-end">
                    ...
                    <button onClick={fetchData} className="h-9 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10 active:scale-95 flex items-center justify-center gap-2">
                        <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Execute PhonePe Audit
                    </button>
                </div>
                <Landmark className="absolute -right-12 -bottom-12 w-48 h-48 text-purple-600/[0.03] pointer-events-none group-hover:rotate-12 transition-transform duration-1000" />
            </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-purple-500" /> PhonePe Transaction Report Manifest
                    </h3>
                    <div className="flex items-center gap-2">
                        <Activity className="w-4 h-4 text-slate-300" />
                        <span className="text-[9px] font-bold uppercase tracking-widest text-purple-600">Settlement Stream Active</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Settlement Type</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Temporal</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="6" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning PhonePe Vault...</td></tr>
                            ) : data.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="py-24 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                            ...
                                            <div>
                                                <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Settlement Matrix Clean</p>
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex flex-col gap-0.5">
                                            ...
                                            <span className="text-[10px] font-bold text-purple-600 uppercase tracking-widest">{row.transaction_id}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-2">
                                            <CardIcon className="w-3.5 h-3.5 text-purple-500" />
                                            <span className="text-[10px] font-bold text-slate-700 uppercase tracking-tight">{row.settlement_type}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-right">
                                        <div className="flex flex-col items-end gap-0.5">
                                            ...
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Settlement Synced</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
...
export default PhonePeTrxReport;
