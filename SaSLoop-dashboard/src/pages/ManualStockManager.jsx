...
import { 
  Box, Search, RefreshCw, Plus, ArrowDownRight, 
  Trash2, X, ClipboardList, DollarSign, Truck
} from "lucide-react";
...
const ManualStockManager = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [rawRes, venRes, logRes] = await Promise.all([
            ...
            const [rawData, venData, logData] = await Promise.all([
                rawRes.json(), venRes.json(), logRes.json()
            ]);
            ...
            if (rawArr.length > 0 && !form.raw_item_id) {
                setForm(prev => ({ 
                    ...
                    unit_price: parseFloat(rawArr[0].unit_cost || rawArr[0].last_purchase_price || 0)
                }));
            }
        } catch (e) {
            console.error("Fetch stock entry data error:", e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRawItemChange = (itemId) => {
        ...
        if (rawObj) {
            setForm(prev => ({
                ...
                unit: rawObj.unit || 'Kg',
                unit_price: parseFloat(rawObj.unit_cost || rawObj.last_purchase_price || 0)
            }));
        }
    };
    ...
    const handleSaveStockIn = async (e) => {
        ...
        try {
            const q = getImpersonateParam();
            const res = await fetch(`${API_BASE}/api/inventory/stock-in${q}`, {
                method: "POST",
            ...
            if (res.ok) {
                ...
                setForm({
                    ...
                    unit_price: parseFloat(rawMaterials[0]?.unit_cost || 0),
                    vendor_id: "",
                    reference_no: "",
                    note: ""
                });
                ...
                alert(`Error: ${err.error || "Failed to log stock inward"}`);
            }
        } catch (e) {
    ...
    const filteredLogs = logs.filter(l => 
        ...
        (l.vendor_name || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
        (l.reference_no || "").toLowerCase().includes(searchQuery.toLowerCase())
    );

    const totalStockInCost = logs.reduce((sum, l) => sum + parseFloat(l.total_cost || 0), 0);
    ...
    return (
        <div className="space-y-4 animate-pro-in pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-[#1e2129] p-4 rounded-xl border border-slate-100 dark:border-white/5 shadow-sm">
                <div>
                    <h2 className="text-[18px] font-black text-slate-900 dark:text-white uppercase tracking-tight">Stock In (Purchase Entry)</h2>
                    <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Record new stock purchases, vendor receipts, and inward inventory logs</p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setIsModalOpen(true)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[11px] font-black uppercase tracking-wider flex items-center gap-1.5 shadow-lg shadow-emerald-600/20">
                        <Plus className="w-4 h-4" /> New Stock In Entry
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-white dark:bg-[#1e2129] p-4 rounded-xl border border-slate-100 dark:border-white/5 flex items-center justify-between">
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Purchases Logged</p>
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white mt-1">{logs.length} <span className="text-[11px] font-bold text-slate-400">Entries</span></h3>
                    </div>
                ...
                <div className="bg-white dark:bg-[#1e2129] p-4 rounded-xl border border-slate-100 dark:border-white/5 flex items-center justify-between">
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Inward Value</p>
                        <h3 className="text-2xl font-black text-emerald-600 dark:text-emerald-500 mt-1">₹{totalStockInCost.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</h3>
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center">
            ...
            <div className="bg-white dark:bg-[#1e2129] p-3 rounded-xl border border-slate-100 dark:border-white/5 flex items-center gap-3">
                <div className="flex items-center gap-2 bg-slate-50 dark:bg-black/20 border border-slate-100 dark:border-white/5 rounded-lg px-3 py-2 flex-1">
                    ...
                    <input 
                        ...
                        placeholder="Search stock inward history by item, vendor or PO reference..." 
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/20 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                            ...
                            <th className="p-3.5">Inward Quantity</th>
                            <th className="p-3.5">Unit Purchase Rate</th>
                            <th className="p-3.5">Total Amount</th>
                            <th className="p-3.5">Vendor / Invoice Ref</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-[11px] font-bold text-slate-800 dark:text-slate-200">
                        {loading ? (
                            <tr><td colSpan="7" className="p-12 text-center text-slate-400 animate-pulse">Loading stock inward history...</td></tr>
                        ) : filteredLogs.length === 0 ? (
                            <tr><td colSpan="7" className="p-12 text-center text-slate-400">No purchase stock entries found. Click "New Stock In Entry" to log purchases.</td></tr>
                            ...
                            <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-3.5 font-black text-emerald-600 dark:text-emerald-500">
                                    +{parseFloat(log.quantity || 0).toLocaleString()} {log.unit || 'Kg'}
                                </td>
                                <td className="p-3.5">₹{parseFloat(log.unit_price || 0).toFixed(2)}</td>
                                ...
                                <td className="p-3.5 text-slate-400 italic">{log.note || 'Purchase inward'}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                    ...
                    <div className="bg-white dark:bg-[#1e2129] border border-slate-100 dark:border-white/10 rounded-2xl w-full max-w-lg p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-white/5">
                            <h3 className="text-[15px] font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                                <ArrowDownRight className="w-5 h-5 text-emerald-500" /> New Stock In (Purchase Entry)
                            </h3>
                        ...
                        <form onSubmit={handleSaveStockIn} className="space-y-4 mt-4 text-[11px] font-bold text-slate-700 dark:text-slate-300">
                            <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Received Quantity *</label>
                                    <input 
                                        type="number" 
                                        step="0.001"
                                        required
                                ...
                                <div>
                                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Unit Purchase Price (₹) *</label>
                                    <input 
                                        type="number" 
                                        step="0.01"
                                        required
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Vendor / Supplier</label>
                                    <select 
                                ...
                                <div>
                                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Invoice / Ref No.</label>
                                    <input 
                                        type="text" 
                                        value={form.reference_no} 
                                        onChange={(e) => setForm({ ...form, reference_no: e.target.value })} 
                                        placeholder="e.g. INV-9021, PO-88"
                            ...
                            <div className="p-3 bg-emerald-500/10 rounded-xl flex items-center justify-between">
                                <span className="text-[10px] font-black uppercase text-emerald-600 dark:text-emerald-500">Total Purchase Cost:</span>
                                <span className="text-[14px] font-black text-emerald-600 dark:text-emerald-500">₹{(parseFloat(form.quantity || 0) * parseFloat(form.unit_price || 0)).toFixed(2)}</span>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-white/5 flex gap-3">
                                ...
                                <button type="submit" disabled={submitting} className="flex-[2] py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-600/20">
                                    {submitting ? "Submitting..." : "Confirm Stock In"}
                                </button>
                            </div>
                        </form>
                    </div>
