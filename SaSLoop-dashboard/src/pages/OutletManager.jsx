    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (res.ok) setData(await res.json());
                ...
                setData([
                    { id: 1, name: "Main CP Outlet", city: "New Delhi", brand: "SaS Burger Co.", status: "Online" },
                    { id: 2, name: "Andheri Hub", city: "Mumbai", brand: "Vibe Coffee", status: "Offline" }
                ]);
            }
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Unit-level operational nodes & deployment telemetry</p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Heartbeat</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Operations</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="5" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning Outlet Vaults...</td></tr>
                            ) : data.map((item, idx) => (
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest opacity-60">ID: OTL_{item.id}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
