...
async function runTest() {
    try {
        ...
        // --- STEP 3: Select "opt_5636" (HALF option selection) ---
        console.log("\n=== STEP 3: Selecting HALF option ===");
        await whatsappManager.handleMetaWebhook(makeWebhookPayload("opt_5636", "msg_3"));
        sess = await getSessionState();
        ...
        await whatsappManager.handleMetaWebhook(makeWebhookPayload("opt_5637", "msg_5"));
        sess = await getSessionState();
        ...
        const omiBefore = await pool.query(
            "SELECT id, item_name, stock_qty FROM outlet_menu_items WHERE id IN (5636, 5637)"
        );
