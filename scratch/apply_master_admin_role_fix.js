...
function applyRoleFix(filePath) {
    if (!fs.existsSync(filePath)) return;
    let content = fs.readFileSync(filePath, 'utf8');

    // 1. Update /outlet-menus
    const oldMenusRegex = /router\.get\("\/outlet-menus"[\s\S]*?res\.json\(result\.rows\);[\s\S]*?\}\);/;
  ...
  try {
    console.log(\`[GET /outlet-menus] User: \${req.user.id}, Role: \${userRole}, Target: \${targetId}\`);
    let query = \`
      ...
      WHERE (
        ...
        om.outlet_id = $2 OR 
        om.user_id = $2 OR 
        om.user_id = (SELECT parent_user_id FROM app_users WHERE id = $2) OR 
        om.user_id IN (SELECT id FROM app_users WHERE parent_user_id = $2) OR
        om.outlet_id IN (SELECT id FROM app_users WHERE parent_user_id = $2)
      )
      ORDER BY om.menu_name ASC
    \`;
    const result = await pool.query(query, [isMaster && (!outlet_id || outlet_id === 'global'), targetId]);
    ...
    // 2. Update /outlet-all-items
    const oldAllItemsRegex = /router\.get\("\/outlet-all-items"[\s\S]*?res\.json\(result\.rows\);[\s\S]*?\}\);/;
    const newAllItemsBlock = `router.get("/outlet-all-items", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (oldMenusRegex.test(content)) {
        content = content.replace(oldMenusRegex, newMenusBlock);
        console.log(`SUCCESS 1: Updated /outlet-menus in ${filePath}`);
    } else {
        console.log(`WARNING 1: /outlet-menus regex match failed in ${filePath}`);
    }
...
applyRoleFix('c:/Users/Sajad/Desktop/SaSLoop/routes/brandRoutes.js');
applyRoleFix('c:/Users/Sajad/Desktop/SaSLoop/pos-app/server/routes/brandRoutes.js');
