...
const filePath = 'scratch/App_reconstructed_context.jsx';
let content = fs.readFileSync(filePath, 'utf8').replace(/\r\n/g, '\n');

// Clean up the broken KOTHistoryView modifiers block which is causing the React parse error
const targetStr = `             <div className="mt-2 pt-2 border-t border-dashed border-white/5 text-[7px] font-bold text-slate-600 uppercase flex justify-between">
...
const replacementStr = `            <div className="mt-2 pt-2 border-t border-dashed border-white/5 text-[7px] font-bold text-slate-600 uppercase flex justify-between">
               <span>{new Date(kot.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
...
if (content.includes(targetStr)) {
  ...
  console.log('Successfully fixed KOTHistoryView syntax error in reconstructed file!');
} else {
  // Try fuzzy replace
  const searchPattern = /const KOTHistoryView =[\s\S]*?const SaSLoopLogo =/;
  const match = content.match(searchPattern);
  if (match) {
    const fixedView = `const KOTHistoryView = ({ tableKots, isDark }) => (
...
// --- POS`;
    content = content.replace(/const KOTHistoryView =[\s\S]*?\/\/ --- POS/, fixedView);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log('Successfully fuzzy-fixed KOTHistoryView syntax error in reconstructed file!');
  } else {
    console.error('KOTHistoryView pattern not found in reconstructed file.');
  }
}
