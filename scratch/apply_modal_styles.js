...
const replacements = [
  ...
  },
  {
    find: `<div className={\`p-5 flex justify-between items-center border-b \${
                       isDark ? 'bg-[#0d1117]/50 border-[#30363d]' : 'bg-slate-55 border-slate-100'
                    }\`}>
  ...
  },
  {
    ...
    replace: `<div className={\`p-6 border-b flex justify-between items-center \${isDark ? 'border-[#30363d] bg-[#161b22]' : 'border-slate-200 bg-slate-50'}\`}>
                        <div>
                           ...
                           <p className="text-[10px] font-bold text-slate-400 mt-1">{trayCustomer?.name || 'Customer'} • {trayFullPhone}</p>
                        </div>
  ...
  {
    find: `className="p-3 border-b border-[#30363d] flex items-center gap-4 text-xs text-[#c9d1d9]"`,
    replace: `className={\`p-3 border-b flex items-center gap-4 text-xs \${isDark ? 'border-[#30363d] text-[#c9d1d9]' : 'border-slate-200 text-slate-750 bg-slate-50/50'}\`}`
  },
  ...
  // 12. Coupon Modal Header Padding
  {
  ...
  // 13. Access Level Modal
  {
    ...
    replace: `className="w-full max-w-4xl bg-[#0d1117] rounded-[2rem] overflow-hidden shadow-2xl border border-[#30363d] flex flex-col h-[85vh]"`
  },
  {
...
replacements.forEach((rep, index) => {
  if (content.includes(rep.find)) {
    ...
    console.warn(`WARNING: Replacement ${index + 1} not found in App.jsx!`);
    console.log(`  Query: ${rep.find.substring(0, 80).replace(/\s+/g, ' ')}...`);
  }
});
...
if (appliedCount > 0) {
  ...
  console.log(`Successfully updated ${appliedCount} style references in App.jsx!`);
} else {
  console.error("ERROR: No replacements were applied.");
}
