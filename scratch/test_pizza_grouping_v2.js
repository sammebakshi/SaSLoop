...
async function runTest() {
    try {
        console.log("Setting mock phone ID for user 8...");
        // Save original meta_phone_id of user 8
        const originalUserRes = await pool.query("SELECT meta_phone_id FROM app_users WHERE id = $1", [userId]);
        const originalPhoneId = originalUserRes.rows[0]?.meta_phone_id;

        await pool.query("UPDATE app_users SET meta_phone_id = $1 WHERE id = $2", [mockPhoneId, userId]);
        ...
        console.log("Restoring original phone ID...");
        await pool.query("UPDATE app_users SET meta_phone_id = $1 WHERE id = $2", [originalPhoneId, userId]);
