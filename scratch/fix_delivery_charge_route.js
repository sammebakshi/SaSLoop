...
function addDeliveryChargeRoute(filePath) {
...
`;
    ...
    if (!content.includes('router.put("/:id/delivery-charge"')) {
        content = content.replace('module.exports = router;', routeCode + '\nmodule.exports = router;');
        fs.writeFileSync(filePath, content, 'utf8');
        console.log(`Added PUT /:id/delivery-charge to ${filePath}`);
    } else {
        console.log(`Route already present in ${filePath}`);
    }
}

addDeliveryChargeRoute('routes/orderRoutes.js');
addDeliveryChargeRoute('pos-app/server/routes/orderRoutes.js');
