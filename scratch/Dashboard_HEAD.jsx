...
function Dashboard() {
  ...
  const unlockAudio = () => {
    ...
    if (newVal) {
      ...
      audio.volume = 0.1;
      audio.play().then(() => {
          alert("≡ƒöö Sound Notifications Enabled Successfully!");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // ≡ƒöö Sound trigger for new orders
        setOrders(prev => {
        ...
        // ≡ƒöö Sound trigger for waiter calls
        ...
        // ≡ƒöö Fetch WhatsApp & System Notifications
    ...
    fetchData();
    const itv = setInterval(fetchData, 3000); // ΓÜí High-speed sync
  ...
  const processCommand = (cmd) => {
    if (cmd.includes("revenue")) setVoiceIntel("Today's revenue is Γé╣45,200. Up 12% from yesterday.");
  ...
  return (
    <div className={`flex flex-col h-full bg-white ${isMobile ? 'p-4 space-y-4' : 'p-6 space-y-8'} overflow-y-auto no-scrollbar`}>
      ...
      <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4'} gap-3 md:gap-6`}>
        {[
          { label: 'Revenue', val: `Γé╣${analytics.totalRevenue.toLocaleString()}`, trend: '+12%', icon: CreditCard, color: 'emerald' },
          { label: 'Orders', val: orders.length, trend: '+4%', icon: ShoppingBag, color: 'blue' },
          { label: 'Avg Sale', val: `Γé╣${analytics.avgOrder}`, trend: '-1%', icon: TrendingUp, color: 'indigo' },
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                         ...
                         <p className="text-[9px] text-slate-400 font-bold uppercase">
                            {new Date(order.created_at).toLocaleDateString([], { day: '2-digit', month: 'short' })} ΓÇó {new Date(order.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                         </p>
                      </div>
                   </div>
                   <div className="text-right">
                      <p className="text-[11px] font-black text-emerald-600">Γé╣{Math.floor(order.total_price)}</p>
                   </div>
                </div>
              ))}
              {orders.length === 0 && (
