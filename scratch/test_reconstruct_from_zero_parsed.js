...
const backupPath = 'C:\\Users\\Sajad\\Desktop\\SaSLoop\\pos-app\\src\\App.jsx'; // May 3rd clean App.jsx
...
const safeParseValue = (val) => {
  ...
  // If it starts with quotes, parse it as a JSON string literal
...
function replaceFuzzy(fileContent, target, replacement, editDesc, chunkIdx) {
  ...
  const targetNorm = normalizeCR(safeParseValue(target));
  const replNorm = normalizeCR(safeParseValue(replacement));
...
fs.writeFileSync('C:\\Users\\Sajad\\Desktop\\SaSLoop\\scratch\\App_reconstructed_parsed.jsx', fileContent, 'utf8');
console.log('Reconstruction finished. Wrote to App_reconstructed_parsed.jsx');
