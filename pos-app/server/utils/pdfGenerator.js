...
function generateTextReceipt(order, biz) {
    ...
    const isDeliveryOrPickupText = order.order_type === 'DELIVERY' || order.order_type === 'PICKUP' || order.source === 'WHATSAPP' || order.source === 'ONLINE_ORDER' || (order.address && order.address.length > 5);
    const textFooter = isDeliveryOrPickupText ? 'Thank you for your order! 🙏✨' : 'Thank you for dining with us! 🙏✨';
...
function generatePdfBuffer(order, biz) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Footer
            const isDeliveryOrPickupPdf = order.order_type === 'DELIVERY' || order.order_type === 'PICKUP' || order.source === 'WHATSAPP' || order.source === 'ONLINE_ORDER' || (order.address && order.address.length > 5);
            const pdfFooter = isDeliveryOrPickupPdf ? 'Thank you for your order! We appreciate your business. 🙏✨' : 'Thank you for dining with us! We appreciate your business. 🙏✨';
            doc.fillColor('#64748B').fontSize(9).font('Helvetica-Oblique').text(pdfFooter, 40, 750, { align: 'center' });
...
function generateThermalPdfBuffer(order, biz) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Footer message based on order type
            const isDeliveryOrPickup = orderType === 'DELIVERY' || orderType === 'PICKUP' || order.source === 'WHATSAPP' || order.source === 'ONLINE_ORDER';
            const footerTitle = isDeliveryOrPickup ? 'THANK YOU FOR YOUR ORDER!' : 'THANK YOU! VISIT AGAIN';

            doc.fillColor('#000000');
            doc.fontSize(9).font('Helvetica-Bold').text(footerTitle, marginX, y, { width: contentWidth, align: 'center' });
            y += 13;
