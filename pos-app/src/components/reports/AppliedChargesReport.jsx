...
import { 
  Coins, Search, Filter, Download, BarChart3, TrendingUp,
  ...
  ArrowRight, Scale, Info, ChevronRight, Printer
} from "lucide-react";
import { API_BASE } from "../../services/api";

const AppliedChargesReport = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/brand/analytics/applied-charges?${q}`, {
    ...
    const handlePrintThermalReport = () => {
        ...
        const totalCharges = data.reduce((a, b) => a + parseFloat(b.charge_amount || 0), 0);

        const chargeRows = data.map(c => `
            <div class="flex-between">
                <span class="bold">ORD#${c.order_id}</span>
                <span>bill #${c.bill_no}</span>
                <span class="bold">₹${parseFloat(c.charge_amount || 0).toFixed(2)}</span>
            </div>
        `).join('');
        ...
        const printHtml = `
            <html>
            <head>
                <title>Applied Charges Report</title>
                <style>
            ...
            <body>
                <div class="center bold" style="font-size: 14px;">APPLIED CHARGES REPORT</div>
                <div class="center bold" style="font-size: 12px; margin-top: 2px;">${outletName.toUpperCase()}</div>
                <div class="dashed-line"></div>
                <div>DATE: ${filters.from_date}</div>
                <div class="dashed-line"></div>
                ...
                <div class="flex-between bold">
                    <span>TOTAL OVERHEAD CHARGES:</span>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">Overhead Audit</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Applied overheads & net order amount variance reconciliations</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button className="px-4 py-2 bg-indigo-600 text-white rounded-md font-bold text-[10px] uppercase tracking-widest hover:bg-indigo-500 transition-all flex items-center gap-2 shadow-md shadow-indigo-600/10">
                        <Download className="w-3.5 h-3.5" /> Export Overheads
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm relative overflow-hidden group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider px-1">Charge Status</label>
                        <select className="w-full h-9 bg-slate-50 border border-slate-200 rounded-md px-3 text-[11px] font-bold uppercase outline-none focus:border-indigo-500 transition-all cursor-pointer" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})}>
                            <option value="Applied">APPLIED OVERHEADS</option>
                            <option value="Reversed">REVERSED CHARGES</option>
                        </select>
                    </div>
                    <div className="space-y-1.5 md:col-span-1">
                        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider px-1">Audit Date</label>
                        <input type="date" className="w-full h-9 bg-slate-50 border border-slate-200 rounded-md px-3 text-[10px] font-bold uppercase outline-none focus:border-indigo-500 transition-all" value={filters.from_date} onChange={e => setFilters({...filters, from_date: e.target.value})} />
                    </div>
                    <button onClick={fetchData} className="h-9 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10 active:scale-95 flex items-center justify-center gap-2">
                        <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Sync Overheads
                    </button>
                </div>
                <Scale className="absolute -right-12 -bottom-12 w-48 h-48 text-indigo-600/[0.03] pointer-events-none group-hover:rotate-12 transition-transform duration-1000" />
            </div>

            {/* Overhead Manifest Theater */}
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-indigo-500" /> Applied Charges Manifest
                    </h3>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Charge Realized</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Net Order Value</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="5" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning Overhead Vaults...</td></tr>
                            ) : data.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="py-24 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                            ...
                                            <div>
                                                <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Overhead Matrix Clean</p>
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex flex-col gap-0.5">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">BILL#{row.bill_no}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[18px] font-bold text-slate-900 tracking-tight">₹{parseFloat(row.charge_amount).toLocaleString('en-IN')}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="text-[18px] font-bold text-slate-900 tracking-tight">₹{parseFloat(row.order_amount).toLocaleString('en-IN')}</span>
                                            <span className="text-[9px] font-bold text-indigo-500 uppercase tracking-widest">Consolidated Total</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-right">
                                        <span className={`px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border ${row.status === 'Applied' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                            {row.status}
                                        </span>
                                    </td>
                                </tr>
...
export default AppliedChargesReport;
