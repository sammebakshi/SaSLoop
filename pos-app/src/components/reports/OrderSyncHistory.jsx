...
import { 
  RefreshCw, Search, Filter, Download, BarChart3, TrendingUp,
  Zap, CheckCircle2, ChevronDown, Monitor, 
  Truck, Smartphone, Globe, Database, ListTree, Settings2, 
  ShieldCheck, ArrowRight, Activity, CloudSync, Clock, Link2, ChevronRight, Printer
} from "lucide-react";
import { API_BASE } from "../../services/api";

const OrderSyncHistory = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/brand/analytics/sync-history?${q}`, {
    ...
    const handlePrintThermalReport = () => {
        ...
        const syncRows = data.map(s => `
            <div class="flex-between">
                <span class="bold">ORD#${s.order_id}</span>
                <span class="bold">${(s.hostbook_status || 'SYNCED').toUpperCase()}</span>
            </div>
        `).join('');
        ...
        const printHtml = `
            ...
            <body>
                <div class="center bold" style="font-size: 14px;">ORDER SYNC HISTORY</div>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">Sync Intelligence</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Hostbook status monitoring & order sync latency audit</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button className="px-4 py-2 bg-blue-600 text-white rounded-md font-bold text-[10px] uppercase tracking-widest hover:bg-blue-500 transition-all flex items-center gap-2 shadow-md shadow-blue-600/10">
                        <Download className="w-3.5 h-3.5" /> Export Sync Audit
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm relative overflow-hidden group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    ...
                    <button onClick={fetchData} className="h-9 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10 active:scale-95 flex items-center justify-center gap-2">
                        <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Execute Sync Audit
                    </button>
                </div>
                <CloudSync className="absolute -right-12 -bottom-12 w-48 h-48 text-blue-600/[0.03] pointer-events-none group-hover:rotate-12 transition-transform duration-1000" />
            </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-blue-500" /> Order Sync History Manifest
                    </h3>
                    <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="text-[9px] font-bold uppercase tracking-widest text-blue-600">Host Connection Stable</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Hostbook Status</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Created Temporal</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Updated Temporal</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="6" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning Sync Vaults...</td></tr>
                            ) : data.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="py-24 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                            <div className="w-20 h-20 rounded-full border-2 border-dashed border-slate-300 flex items-center justify-center">
                                                <CloudSync className="w-10 h-10 text-slate-400" />
                                            </div>
                                            <div>
                                                <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Sync Matrix Clean</p>
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-blue-50 border border-blue-100 rounded-lg flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-sm">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded text-[9px] font-bold uppercase tracking-widest">{row.hostbook_status}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6">
                                    ...
                                    <td className="px-6 py-6 text-right">
                                        <button className="p-2 bg-slate-50 border border-slate-200 text-slate-400 rounded-md hover:bg-slate-900 hover:text-white transition-all shadow-sm"><RefreshCw className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
...
export default OrderSyncHistory;
