...
import { 
  Percent, Search, Filter, Download, BarChart3, TrendingUp,
  Zap, CheckCircle2, RefreshCw, ChevronDown, Monitor, 
  Truck, Smartphone, Globe, Database, ListTree, Settings2, 
  ShieldCheck, ArrowUpRight, Scissors, Tag, ShoppingBag, Utensils, ChevronRight, Printer
} from "lucide-react";
...
const DiscountReport = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/brand/analytics/discount-report?${q}`, {
    ...
    const handlePrintThermalReport = () => {
        ...
        const totalDiscount = data.reduce((a, b) => a + parseFloat(b.discount_amount || 0), 0);

        const discountRows = data.map(d => `
            <div class="flex-between">
                <span class="bold">${(d.order_from || 'DINE_IN').toUpperCase()}</span>
                <span class="bold">₹${parseFloat(d.discount_amount || 0).toFixed(2)}</span>
            </div>
        `).join('');
        ...
        const printHtml = `
            ...
            <body>
                <div class="center bold" style="font-size: 14px;">DISCOUNT SUMMARY REPORT</div>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">Promotion Audit</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Realized discount volume tracking & promotion auditing</p>
                    </div>
                </div>
            ...
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm relative overflow-hidden group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    ...
                    <button onClick={fetchData} className="h-9 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10 active:scale-95 flex items-center justify-center gap-2">
                        <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Analyze Discounts
                    </button>
                </div>
                <Percent className="absolute -right-12 -bottom-12 w-48 h-48 text-rose-600/[0.03] pointer-events-none group-hover:rotate-12 transition-transform duration-1000" />
            </div>

            {/* Discount Report Matrix */}
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-rose-500" /> Discount Report Manifest
                    </h3>
                    <div className="flex items-center gap-3">
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Note: Fulfilled & Cancelled manifests only</span>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Channel Origin</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Discount Volume</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="4" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning Promo Vaults...</td></tr>
                            ) : data.length === 0 ? (
                                <tr>
                                    <td colSpan="4" className="py-24 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                            ...
                                            <div>
                                                <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Promo Matrix Clean</p>
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="text-[18px] font-bold text-rose-600 tracking-tight">₹{parseFloat(row.discount_amount).toLocaleString('en-IN')}</span>
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Realized Volume</span>
                                        </div>
                                    </td>
