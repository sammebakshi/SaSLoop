...
import { 
  ...
  PackageCheck, Bike, ChevronRight, Printer
} from "lucide-react";
...
const DeliveryReport = () => {
    ...
    const totalAmount = data.reduce((a, b) => a + parseFloat(b.total || 0), 0);

    const handlePrintThermalReport = () => {
        if (data.length === 0) return;
        const outletName = outlets.find(o => String(o.id) === String(filters.outlet_id))?.name || 'OUTLET';

        const deliveryRows = data.map(d => `
            <div class="flex-between">
                <span class="bold" style="max-width: 120px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${(d.customer_name || 'GUEST').toUpperCase()}</span>
                <span>ord #${d.unique_id}</span>
                <span class="bold">₹${parseFloat(d.total || 0).toFixed(2)}</span>
            </div>
        `).join('');
        ...
        const printHtml = `
            ...
            <body>
                <div class="center bold" style="font-size: 14px;">DELIVERY FULFILLMENT REPORT</div>
