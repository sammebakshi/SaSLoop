...
import { 
  ShieldAlert, Search, Filter, Download, BarChart3, TrendingUp,
  Zap, CheckCircle2, RefreshCw, ChevronDown, Monitor, 
  Truck, Smartphone, Globe, Database, ListTree, Settings2, 
  ShieldCheck, Lock, Key, UserCheck, AlertTriangle, ArrowRight,
  Fingerprint, ChevronRight, Printer
} from "lucide-react";
import { API_BASE } from "../../services/api";

const PasscodeUserReport = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/brand/analytics/passcode-history?${q}`, {
    ...
    const handlePrintThermalReport = () => {
        ...
        const passcodeRows = data.map(p => `
            <div class="flex-between">
                <span class="bold">${(p.activity || 'OVERRIDE').toUpperCase()}</span>
                <span>code: ${p.passcode}</span>
            </div>
            <div class="flex-between">
                <span>ORD#${p.order_id}</span>
                <span>${new Date(p.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>
        ...
        const printHtml = `
            ...
            <body>
                <div class="center bold" style="font-size: 14px;">SECURITY AUDIT REPORT</div>
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-bold text-slate-800 uppercase tracking-tight">Security Audit Hub</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Passcode tracking & POS billing security matrix</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <button className="px-4 py-2 bg-rose-600 text-white rounded-md font-bold text-[10px] uppercase tracking-widest hover:bg-rose-500 transition-all flex items-center gap-2 shadow-md shadow-rose-600/10">
                        <Download className="w-3.5 h-3.5" /> Export Security Audit
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm relative overflow-hidden group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    ...
                    <button onClick={fetchData} className="h-9 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-widest rounded-md hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10 active:scale-95 flex items-center justify-center gap-2">
                        <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} /> Execute Security Audit
                    </button>
                </div>
                <ShieldAlert className="absolute -right-12 -bottom-12 w-48 h-48 text-rose-600/[0.03] pointer-events-none group-hover:rotate-12 transition-transform duration-1000" />
            </div>

            {/* Security Manifest Theater */}
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-rose-500" /> Passcode History Manifest
                    </h3>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                ...
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Security Activity</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Terminal Origin</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Auth Passcode</th>
                                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Document Artifacts</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="5" className="py-24 text-center text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] animate-pulse">Scanning Security Vaults...</td></tr>
                            ) : data.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="py-24 text-center">
                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                            ...
                                            <div>
                                                <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Security Matrix Clean</p>
                                ...
                                <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 bg-rose-50 border border-rose-100 rounded-lg flex items-center justify-center text-rose-600 group-hover:bg-rose-600 group-hover:text-white transition-all shadow-sm">
                                                <Fingerprint className="w-5 h-5" />
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="font-bold text-slate-800 uppercase tracking-tight text-[13px]">{row.activity}</span>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="px-2 py-0.5 bg-slate-900 text-white text-[8px] font-bold uppercase tracking-widest rounded">{row.requested_permission}</span>
                                    ...
                                    <td className="px-6 py-6">
                                        <div className="flex flex-col gap-0.5">
                                            <span className="text-[12px] font-bold text-slate-700 uppercase tracking-wider">{row.pos_billing}</span>
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{row.outlet_name}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-6 text-center">
                                        <span className="font-bold text-slate-400 uppercase tracking-[0.2em] group-hover:text-rose-600 transition-colors underline decoration-slate-200 decoration-dashed underline-offset-4">
                                            {row.passcode}
                                        </span>
                                    </td>
                                    <td className="px-6 py-6 text-right">
                                        <div className="flex flex-col items-end gap-1">
                                            <span className="text-[10px] font-bold text-slate-800 uppercase tracking-tight">ORD#{row.order_id} | BILL#{row.bill_no}</span>
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">KOT REF: {row.kot_no}</span>
                                        </div>
                                    </td>
                                </tr>
                            ))}
...
export default PasscodeUserReport;
