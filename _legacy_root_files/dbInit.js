...
async function initializeDatabase() {
    try {
        ...
        const queries = [
            ...
            `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'app_users_username_key') THEN ALTER TABLE app_users ADD CONSTRAINT app_users_username_key UNIQUE (username); END IF; END $$`,
            `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'app_users_phone_key') THEN ALTER TABLE app_users ADD CONSTRAINT app_users_phone_key UNIQUE (phone); END IF; END $$`,
