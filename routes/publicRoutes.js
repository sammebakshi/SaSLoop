...
router.get("/table-status/:userId/:tableName", async (req, res) => {
    try {
        ...
        if (activePosState) {
            ...
            // A table is ONLY occupied in POS draft state if there is at least one active, un-cancelled item
            const matchedBillKey = Object.keys(tableBills).find(k => isMatchingKey(k));
            if (matchedBillKey && Array.isArray(tableBills[matchedBillKey])) {
                const uncancelledItems = tableBills[matchedBillKey].filter(i => i && !i.isCancelled);
