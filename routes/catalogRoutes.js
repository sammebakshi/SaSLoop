...
router.post("/", authMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [req.user.bizId, code, product_name, category, sub_category, price, availability, image_url || null, description || null, tax_applicable !== undefined ? tax_applicable : 1, is_veg || false, stock_count !== undefined && stock_count !== '' && stock_count !== null ? parseInt(stock_count) : null]
        );
    ...
    try {
        ...
        for (const item of items) {
            ...
            await client.query(
                ...
                [req.user.bizId, item.code, item.product_name, item.category, item.sub_category, item.price, item.availability, item.image_url || null, item.description || null, item.tax_applicable !== undefined ? item.tax_applicable : 1]
            );
            successCount++;
        }
...
router.put("/:id", authMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [code, product_name, category, sub_category, price, availability, image_url, description, tax_applicable !== undefined ? tax_applicable : 1, is_veg || false, stock_count !== undefined && stock_count !== '' && stock_count !== null ? parseInt(stock_count) : null, req.params.id, req.user.bizId]
        );
